...
fn make_mappability(m: &ArgMatches) -> anyhow::Result<Option<Mappability>> {
    if let Some(map) = m.get_one::<PathBuf>("mappability") {
        debug!("Loading mappabililty information from {}", map.display());
        let map_list = Mappability::new(region_list_from_bed(map, None)?);
        Ok(Some(map_list))
    } else {
        Ok(None)
    }
